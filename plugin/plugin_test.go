...
func TestService_Discover(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "doorman-test-*")
	require.NoError(t, err)
	defer os.RemoveAll(tmpDir)

	for _, name := range []string{"doorman-vault", "doorman-keepassxc", "other-tool"} {
	...
	assert.Contains(t, plugins, "doorman-vault")
	assert.Contains(t, plugins, "doorman-keepassxc")
...
func TestService_GetInfo(t *testing.T) {
	...
	svc := &service{}
	info, err := svc.GetInfo("doorman-test")
	require.NoError(t, err)

	assert.Equal(t, "doorman-test", info.Name)
...
func TestService_GetInfoNotFound(t *testing.T) {
	...
	svc := &service{}
	_, err := svc.GetInfo("doorman-nonexistent")
	assert.Error(t, err)
...
func TestService_GetNotFound(t *testing.T) {
	...
	svc := &service{}
	err := svc.Get("doorman-nonexistent", "some/key", &bytes.Buffer{})
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "not found in PATH")
}
